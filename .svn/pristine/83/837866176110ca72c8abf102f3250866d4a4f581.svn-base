...
function userBalance(param){
    ...
    var url = "http://192.168.9.74/bhms/webservice/getUserInfo.action";
    var data="card="+smartcardid+"&cardType=I";
    var sucFunc=param.sucFunc;
    var failFunc=param.failFunc;
    ajax2({url:url,data:data,method:"post",handler:function(xmlHttp){
        var result = eval("("+xmlHttp.responseText+")");
        if(result.code==200){
            sucFunc(result);
        }else{
...
var list = {
    ...
    refresh:function(){
        for(var i=0;i<this.size;i++){
            ...
            $("movie_"+i).getElementsByTagName("img")[0].src="../static/epgimg/"+this.data[i].identify+"_preview.jpg";
    ...
    enter:function(){
        //alert("enter");
        insRecord({action:17,previewid:list.data[list.index].id});//预告片播放
		setTimeout(function(){
			trxnstep=2;
			var url = iPanel.eventFrame.pre_epg_url+"/defaultHD/en/hidden_byTime_gehua.jsp?typeId=-1&vodId="+list.data[list.index].mediaid+"-sfyx"+"&playType=1&idType=FSN&baseFlag=2&appBackUrl="+location.href;
			$("data_ifm").src = url;
		},500);
...
function showTipWindow (mes,type){
    if(trxnstep==0){
        ...
        if(type==2){
            ...
            windows.hidden();
            $("wxtsmsg").innerHTML="询价出错：["+'参数错误或影片不存在'+"]";
            windows.show("wxts");
        }
        ...
        /*订购出错*/
        windows.hidden();
        $("wxtsmsg").innerHTML="订购出错：["+'参数错误或影片不存在'+"]";
        windows.show("wxts");
    }
}
...
window.onload=function(){
    ...
    focusIndex=2;
    /* userBalance({sucFunc:function(result){
        alert(result.custInfo.totleBalance)
