...
var URL = {
    ...
    activity:"/REST/activity/list"//活动列表
}
...
var list = {
    ...
    refresh:function(){
        for(var i=0;i<this.size;i++){
            ...
            $("movie_"+i).getElementsByTagName("img")[0].src="../static/epgimg/"+this.data[i].identify+"_usermanage.jpg";
            if(topIndex==0){
	...
	ok:function(){
		if(topIndex==0){
			...
			setTimeout(function(){
				location.href="moviedetail.jsp?pid="+list.data[list.index].id;
			},500);
		}else if(topIndex==1){
			insRecord({action:14,mediaId:this.data[this.index].pid});
			setTimeout(function(){
				location.href="moviedetail.jsp?pid="+list.data[list.index].pid;
			},500);
		}
	}
}
...
function changeTop(){
	if(topIndex==0||topIndex==1){
		...
		getActivity({pageNo:1,success:function(result){
	    	fxjc.init(result);
	    },failure:function(){
...
function getActivity(param){
	var url = URL.ipaddr+URL.activity;
    var pageNo = param.pageNo;
    var pageRow = 4;
    var data="{\"pageNo\":"+pageNo+",\"pageRow\":"+pageRow+"}"; 
    var sucFunc=param.success;
    var failFunc=param.failure;
    ajax({url:url,data:data,method:"post",handler:function(xmlHttp){
        var result = eval("("+xmlHttp.responseText+")");
        if(result.errCode==0){
            sucFunc(result.pageBean);
        }else{
            failFunc();
        }
    }});	
